...
import { profileTable, articlesTable, projectsTable, experienceTable, educationTable } from "@workspace/db";
...
async function seed() {
  ...
  for (const article of articles) {
    const existing = await db.select().from(articlesTable).where(eq(articlesTable.slug, article.slug)).limit(1);
  ...
  const existingExp = await db.select().from(experienceTable).limit(1);
  ...
  const existingEdu = await db.select().from(educationTable).limit(1);
